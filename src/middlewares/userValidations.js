...
const userValidations = [
    ...
    check('email')
        ...
        .custom(async (value, { req }) => {
            ...
            if (userEmail !== null && userEmail.dataValues.email === value.toLowerCase()) {
    ...
    check('emailRespaldo')
        ...
        .custom(async (value, { req }) => {
            ...
            if(email === emailRespaldo){
                throw new Error ('El email de respaldo tiene que ser distinto al email')
            }
            return true
        }),
    check('password')
    ...
    check('genero_id')
        .notEmpty().withMessage('Debes elegir tu genero'),
    check('dni')
        .notEmpty().withMessage('Tienes que completar con tu dni').bail()
        .isNumeric().withMessage('Tienes que poner tu dni').bail()
        .isLength({min: 8}).withMessage('Esto no es un dni').bail()
        .isLength({max: 8}).withMessage('Esto no es un dni'),
    check('birthday')
        .notEmpty().withMessage('Tienes que completar con tu fecha de nacimiento').bail()
        .custom((value, { req }) => {
